...
```cpp
int fd = open("foo", O_CREAT | O_WRONLY | O_TRUNC) // 使用open系统调用创建一个新文件 
```

- open系统调用最重要的是其返回值 {++文件描述符++}
    - 文件描述符是一个整数, 是每个进程{==私有==}的
    - 用户可以通过文件描述符读取或者写入文件

{++读写文件++}

![alt text](./images/读写文件.png)

- 再读取文件前会{++优先++}使用open系统调用打开文件
    - open返回3而不是1的原因 : {++每个正在运行的进程都会打开3个文件:标准输入,标准输出,标准错误++}
- 之后使用read()系统调用重复读取文件中的一些字节
    - read()调用的第一个参数是{==文件描述符==}
    - 一个进程可以同时打开多个文件
    - 第二参数会要读入的数据
    - 第三个参数为 {++读缓冲区,图示大小为4KB++}
- 使用write系统调用往标准输出(文件描述符1)输出内容
- 使用close系统调用关闭文件
- 写文件和读文件类型, 都需要{++先使用open系统调用打开一个文件++}

{++删除文件++}

![alt text](./images/删除文件.png)

- 使用rm仅会调用unlink系统调用, 仅但该inode号中的引用计数为0时候才会释放与该文件相对应的inode和数据块
### 文件的逻辑结构
...
{++文件系统的挂载(mounting)++} 文件系统在使用前都{++需要先进行挂载++},将设备的文件系统挂载到某个目录后,就可以通过这个目录来访问设备(逻辑设备)上的文件.
