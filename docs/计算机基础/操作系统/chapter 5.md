...
![alt text](./images/交叉编号.png)

- 交替编号
    - 对同一个磁盘的扇区,并不按照顺序编号,而是才有隔一位的方式进行编号
        - 减少磁盘的读取延迟,由于磁道读完一个扇区后{++并不能立刻读取下一个扇区++}需要等待磁头恢复
        - 连续读取连续变化会导致磁头白白多选择一周

- 磁盘地址使用 {++柱面,盘面,扇区++} 而不是使用 {--盘面,柱面,扇区--}
    - 这是由于在读取连续扇区的时候, 低地址扇区会位于不同盘面上,此时只需要切换磁头.如果柱面号在中间则需要旋转磁盘,旋转磁头是机械操作远比切换磁头慢

![alt text](./images/磁盘并行交叉编址.png)

- 交叉编号
    - 同一柱面的扇区并不同一编号而是采用交叉编号
