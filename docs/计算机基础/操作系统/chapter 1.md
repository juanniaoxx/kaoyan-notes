...
^^脱机I/O方式^^ 实现将装有用户程序和数据的纸带装入输入机,在一台{++外围机++}的控制下,把纸带上的程序和数据输入磁带.
...
{++时钟中断++} 由{++硬件++}实现外部中断,而非异常, 其设计初心是为了让操作系统能够{++定期获取CPU++}

{++上下文切换(context switch)++} 操作系统为被时钟中断暂停的进程保存一些内容,并为即将由调度程序决定上CPU执行的进程恢复一些资源.

![alt text](./images/时钟中断的例子.png)

- 在发生时钟中断的时候, 运行进程的用户寄存器(PC,通用寄存器组等)被{++硬件++}隐式保持进该进程的内核栈
- 在OS决定从A切换到B的时候, 则是由OS显示保存进用户的进程内核栈中