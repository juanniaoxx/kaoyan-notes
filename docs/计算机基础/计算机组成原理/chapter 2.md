...
![alt text](./images/原码不恢复余数硬件逻辑图.png)
...
4.  {++舍入++} 
    ...
    - 就近舍入 取最近可表示数,若处于中间则取{++偶数++}
        - 适应粘位可以可以减伤就近舍入刚好位于中间的情况
    - 朝 $0$ 舍入, 直接丢弃多余位,即截断法.
    - 数值越大舍入误区越大
        - 数值越大,越远离原点相邻的数之间可以表示的数的间隔就越大
