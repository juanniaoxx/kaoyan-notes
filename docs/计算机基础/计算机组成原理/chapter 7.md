...
把对一个数据块以读/写为单元进行干预(DMA方式),减少为对{++一组++}数据块以读/写及有关控制和管理为单位的干预. 可以实现 {++CPU, 通道和I/O设备++}的并行操作. 

通道通过执行{++通道程序++}与{++设备控制器++}共同工作来实现对I/O设备的控制.

- 通道具有{++独立++}的通道指令系统,通道指令与一般机器指令不同
- 当通道执行完响应的通道程序后,会发出{++中断++}请求来表示I/O管理结束,CPU响应请求,执行响应的中断服务程序进行处理