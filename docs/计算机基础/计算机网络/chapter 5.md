...
- 需要主要在DHCP过程中,仅在{++服务器回应ACK报文后++}主机才能使用分配的IP地址作为源地址
    - 其余时期均使用 0.0.0.0作为报文源IP地址

- 在MAC层客户端发往服务器的基本都采用{++广播++}, 而服务器发会客户端的一般采取{++单播++}
    - 在DHCP DISCOVER报文中, 基本会封装事物ID和客户端MAC地址
    - 通过DHCP DISCOVER报文中MAC地址字段服务器就能知道客户端的MAC地址
    - 事务ID字段用于客户端/服务器确定收到的报文是否是本次DHCP对应方发送过来

- 广播与单播的选择
    - 由于有些TCP/IP的协议栈实现不接受单播, DHCP通过设置 {++BROADCAST=1++} 明确告诉服务器之后的通信都用广播
    - 通过设置{++BROADCAST=0++} 表示可以接受单播通信
