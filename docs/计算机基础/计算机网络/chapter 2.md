...
^^码元速率(波特率,调制速率)^^ 表示单位时间内信号波形的变化次数,即通过信道传输的码元个数. 若信号的码元宽度为 $T$, 单位为 $s$, 则其码元速率 $B=1/T$ ,单位为波特(baud) 
...
^^时延带宽积^^ 单向传播时延与带宽(数据传输速率)的乘积

{++发送时延(传播时延)++} 主机或路由器发送分组所需要耗费的时间

$$
发送时延=\frac{分组长度(b)}{发送速率\ (b/s)}
$$

{++传播时延++} 电磁波在连输(传输介质)上传播一定的距离所耗费的时间

$$
传播时延 = \frac{链路长度(m)}{电磁波在连输上的传播速度\ (m/s)}
$$

- 电磁波在自由空间的传播速度约为 $3\times 10^8 m/s$
- 电磁波在铜线电缆中的传播速率约为 $2.3\times 10^8 m/s$
- 电磁波在光纤中的传播速率约为 $2\times 10^8 m/s$

{++排队时延和处理时延++} 分组在路由器进行排队等待处理和路由器处理报文的时间.

![alt text](./images/时延.png)
